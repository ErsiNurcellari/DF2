...
function isUpToDate($force_check = false)
{
    ...
    $compare = version_compare(VERSION, $remoteVersion, 'ge');
...
function get_base_url($path = '')
{
    ...
    $base = ltrim($server_request_scheme . '://' . $server_name . str_replace('index.php', '', $_SERVER['PHP_SELF']), '/');
...
{
    if ($gateway == 'stripe') {
        if (setting('stripe.enabled', 'yes') == 'yes') {
            if (
                (setting('stripe.sandbox_mode', 'yes') == 'yes' && (setting('stripe.sandbox.pk') == '' || setting('stripe.sandbox.sk') == '')) ||
                (setting('stripe.sandbox_mode', 'yes') == 'no' && (setting('stripe.live.pk') == '' || setting('stripe.live.sk') == ''))
            ) {
                return false;
            } else {
                return true;
    ...
    if ($gateway == 'razorpay') {
        if (setting('razorpay.enabled', 'yes') == 'yes') {
            if (
                (setting('razorpay.sandbox_mode', 'yes') == 'yes' && (setting('razorpay.sandbox.ki') == '' || setting('razorpay.sandbox.sk') == '')) ||
                (setting('razorpay.sandbox_mode', 'yes') == 'no' && (setting('razorpay.live.ki') == '' || setting('razorpay.live.sk') == ''))
            ) {
                return false;
            } else {
                return true;
    ...
    if ($gateway == 'paypal') {
        ...
        if (setting('paypal.enabled', 'yes') == 'yes') {
            if (
                (setting('paypal.sandbox_mode', 'yes') == 'yes' && (setting('paypal.sandbox.username') == '' || setting('paypal.sandbox.password') == '' || setting('paypal.sandbox.signature') == '')) ||
                (setting('paypal.sandbox_mode', 'yes') == 'no' && (setting('paypal.live.username') == '' || setting('paypal.live.password') == '' || setting('paypal.live.signature') == ''))
            ) {
                return false;
            } else {
                return true;
    ...
    if ($gateway == 'offline_payments') {
        if (setting('offline_payments.enabled') == 'yes' &&
        ...
        foreach ($categories as $category) {
            if ($category->product_count > 0) {
                ...
                <?php if ($category->childs->count() > 0) { ?>
                    <li class="<?php echo $childm == true ? 'dropdown-submenu' : 'dropdown'; ?>">
                        <a class="dropdown-toggle"
                           href="<?php echo $category->route; ?>"><?php echo $category->name; ?> <?php if ($childm == false) { ?>
                                <b class="caret"></b><?php } ?>
                        </a>
                            ...
                            <?php foreach ($category->childs as $child) { ?>
                                ...
                                if ($child->childs->count() > 0) {
                                    ...
                                    <li class="dropdown-submenu"><a class="dropdown-toggle"
                                                                    href="<?php echo $child->route; ?>"><?php echo $child->name; ?></a>
                                        <?php
...
{
    $parents = DB::select(DB::raw('SELECT T2.id
                        FROM (
                            SELECT
                                @r AS _id,
...
function site_url($path = null)
{
    ...
    return $path == null ? \setting('app.url') : rtrim(\setting('app.url'), '/') . $path;
}
    ...
    if (setting('recaptcha.enabled') == 'on' && setting('recaptcha.api_secret_key') != null) {
        ...
        if (!isset($response->success) || $response->success != true)
        {
            throw ValidationException::withMessages([
                'recaptcha' => [trans('auth.recaptcha_failed')],
            ]);
        }
    }
}